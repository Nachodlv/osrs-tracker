...
- Hide a node for a profile by setting `state.removed[id] = true`; `pruneRemoved` deletes it and cascades to now-parentless descendants. `removed` is keyed by plain node id. New-save-only defaults belong in `defaultState()` (see `defaultRemoved()`); existing saves load their own stored `removed`, so they are untouched.
